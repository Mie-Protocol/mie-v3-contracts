...
import '@Mieswap/v3-periphery/contracts/base/PeripheryImmutableState.sol';
import '@Mieswap/v3-periphery/contracts/base/BlockTimestamp.sol';
import '@Mieswap/v3-periphery/contracts/libraries/Path.sol';
import '@Mieswap/v3-periphery/contracts/libraries/PoolAddress.sol';
import '@Mieswap/v3-core/contracts/interfaces/IMieV3Pool.sol';
import '@Mieswap/v3-periphery/contracts/libraries/OracleLibrary.sol';
...
abstract contract OracleSlippage is IOracleSlippage, PeripheryImmutableState, BlockTimestamp {
    ...
    function getPoolAddress(
        ...
        address tokenB,
        uint24 fee
    ) internal view virtual returns (IMieV3Pool pool) {
        pool = IMieV3Pool(PoolAddress.computeAddress(deployer, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
    ...
    function getSyntheticTicks(
        ...
        for (uint256 i = 0; i < numPools; i++) {
            ...
            IMieV3Pool pool = getPoolAddress(tokenIn, tokenOut, fee);
            ...
            if (secondsAgo == 0) {
                ...
                (, currentTick, , , , , ) = IMieV3Pool(pool).slot0();
            }
