...
import './MieStableSwapLP.sol';

contract MieStableSwapLPFactory is Ownable {
    ...
    function createSwapLP(
        ...
        // create LP token
        bytes memory bytecode = type(MieStableSwapLP).creationCode;
        uint256 chainId;
        assembly {
            chainId := chainid()
        }
        ...
        MieStableSwapLP(lpToken).setMinter(_minter);
