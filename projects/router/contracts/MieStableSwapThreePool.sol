...
contract MieStableSwapThreePool is Ownable, ReentrancyGuard {
    ...
    // }
    function get_virtual_price() external view returns (uint256) {
        // Calculate D, total virtual value of the portfolio
        uint256 D = get_D(_xp(), get_A());

        // Retrieve total token supply
        uint256 token_supply = token.totalSupply();

        // Calculate virtual price (scaled by 1e18)
        return (D * PRECISION) / token_supply;
    }
