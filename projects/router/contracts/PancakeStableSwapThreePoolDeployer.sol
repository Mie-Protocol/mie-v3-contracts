...
import './MieStableSwapThreePool.sol';

contract MieStableSwapThreePoolDeployer is Ownable {
    ...
    //     bytes memory bytecode = type(MieStableSwapThreePool).creationCode;
    //     uint256 chainId;
    //     assembly {
    //         chainId := chainid()
    //     }
    ...
    //     MieStableSwapThreePool(swapContract).initialize(coins, _A, _fee, _admin_fee, _admin, _LP);
    ...
    function initializeSwapContract(address _swapContract, SwapDeploymentParams memory params) internal {
        ...
        MieStableSwapThreePool(_swapContract).initialize(
