...
import { TestMieV3SwapPay } from '../typechain-types/contracts/test/TestMieV3SwapPay'
...
import { TestMieV3ReentrantCallee } from '../typechain-types/contracts/test/TestMieV3ReentrantCallee'
...
describe('MieV3Pool', () => {
  ...
  let factory: MieV3Factory
  let pool: MockTimeMieV3Pool

  let swapTarget: TestMieV3Callee
  ...
  async function initializeAtZeroTick(pool: MockTimeMieV3Pool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestMieV3ReentrantCallee')
      ).deploy()) as TestMieV3ReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestMieV3SwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestMieV3SwapPay')
      underpay = (await underpayFactory.deploy()) as TestMieV3SwapPay
