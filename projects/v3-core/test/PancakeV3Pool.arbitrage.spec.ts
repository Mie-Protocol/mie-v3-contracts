...
import { MieV3PoolSwapTest } from '../typechain-types/contracts/test/MieV3PoolSwapTest'
...
describe('MieV3Pool arbitrage tests', () => {
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          const arbTestFixture = async ([wallet, arbitrageur]: Wallet[]) => {
            ...
            const testerFactory = await ethers.getContractFactory('MieV3PoolSwapTest')
            const tester = (await testerFactory.deploy()) as MieV3PoolSwapTest
