...
describe('MieV3Factory', () => {
  let wallet: Wallet, other: Wallet

  let deployer: MieV3PoolDeployer
  ...
  const fixture = async () => {
    ...
    const deployer_ = (await deployerFactory.deploy()) as MieV3PoolDeployer
    const factory_ = (await factoryFactory.deploy(deployer_.address)) as MieV3Factory

    await deployer_.setFactoryAddress(factory_.address)
    return [factory_, deployer_] as [MieV3Factory, MieV3PoolDeployer]
  }
  ...
  before('load pool bytecode', async () => {
    poolBytecode = (await ethers.getContractFactory('MieV3Pool')).bytecode
  })
  ...
  async function createAndCheckPool(
    ...
    const poolContractFactory = await ethers.getContractFactory('MieV3Pool')
