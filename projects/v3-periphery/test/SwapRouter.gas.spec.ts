import { abi as IMieV3PoolABI } from '@Mieswap/v3-core/artifacts/contracts/interfaces/IMieV3Pool.sol/IMieV3Pool.json'
import { Fixture } from 'ethereum-waffle'
import { BigNumber, constants, ContractTransaction, Wallet } from 'ethers'
import { ethers, waffle } from 'hardhat'
import { IMieV3Pool, IWETH9, MockTimeSwapRouter, TestERC20 } from '../typechain-types'
...
describe('SwapRouter gas tests', function () {
  ...
  const swapRouterFixture: Fixture<{
    ...
    pools: [IMieV3Pool, IMieV3Pool, IMieV3Pool]
    ...
    const pools = poolAddresses.map((poolAddress) => new ethers.Contract(poolAddress, IMieV3PoolABI, wallet)) as [
      IMieV3Pool,
      IMieV3Pool,
      IMieV3Pool
    ]
  ...
  let pools: [IMieV3Pool, IMieV3Pool, IMieV3Pool]
  ...
  describe('#exactInput', () => {
    ...
    it.skip('0 -> 1 minimal', async () => {
      const calleeFactory = await ethers.getContractFactory('TestMieV3Callee')
